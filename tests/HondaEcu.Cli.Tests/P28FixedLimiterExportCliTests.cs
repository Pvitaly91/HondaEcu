...
public sealed class P28FixedLimiterExportCliTests
{
    private static string[] Args(P28AcquisitionCliTests.Workspace w, string operation)
    {
        w.WriteLineagePlaceholders();
        var common = new[] { "research", "p28-limiter", "export", operation, w.Baseline, "--profile", "p28-304", "--baseline-binding", w.Binding,
            "--compensation-definition", w.Definition, "--output", w.Output };
        return operation switch
        {
            "plan" => [.. common, "--confirm-profile", "--cut-raw", "540", "--resume-raw", "556"],
    ...
    [InlineData("plan", "--cut-raw")]
    [InlineData("plan", "--resume-raw")]
    ...
    {
        using var w = new P28AcquisitionCliTests.Workspace(); var args = Args(w, "plan"); args[Array.IndexOf(args, "--cut-raw") + 1] = raw;
