...
public sealed class P28IgnitionMapExportCliTests
{
    private static string[] Arguments(P28AcquisitionCliTests.Workspace workspace, string operation)
    {
        ...
        var settings = Path.Combine(workspace.Root, "ignition-settings.json");
        File.WriteAllText(settings, """{"formatVersion":1,"purpose":"explicit-ignition-map-cell-values","ignition_map_0":[{"row":0,"column":0,"rawValue":17}],"ignition_map_1":null}""");
        var common = new[] { "research", "p28-ignition", "export", operation, workspace.Baseline,
            "--profile", "p28-304", "--baseline-binding", workspace.Binding,
        ...
        return operation switch
        {
            ...
            "apply" => [.. common, "--confirm-profile", "--confirm-pc-only", "--plan", workspace.Plan,
                "--runner", workspace.Runner, "--saved-plan", Path.Combine(workspace.Root, "new-ignition-plan.json"),
                "--report", Path.Combine(workspace.Root, "new-ignition-receipt.json")],
    ...
    public async Task EveryRouteRequiresItsClosedInputs(string operation, string missing)
    {
        ...
        var arguments = Arguments(workspace, operation).ToList(); arguments.RemoveRange(arguments.IndexOf(missing), 2);
    ...
    public async Task ConfirmationsStayExplicit(string operation, string flag)
    {
        ...
        Assert.Equal(CliApplication.UsageError,
            (await workspace.RunAsync(Arguments(workspace, operation).Where(value => value != flag).ToArray())).Code);
        Assert.False(File.Exists(workspace.Output));
    }
    ...
    [InlineData("--scenario")]
    public async Task NoArbitraryPatchUnitsOrValidationEscapeHatch(string option)
    {
        using var workspace = new P28AcquisitionCliTests.Workspace();
        Assert.Equal(CliApplication.UsageError,
            (await workspace.RunAsync([.. Arguments(workspace, "apply"), option, "1"])).Code);
        Assert.False(File.Exists(workspace.Output));
    }
    ...
    public async Task DestinationAliasingAndCancellationLeaveInputsIntact()
    {
        ...
        Assert.NotEqual(CliApplication.Success,
            (await workspace.RunAsync(Arguments(workspace, "apply"), cancellation.Token)).Code);
        Assert.False(File.Exists(workspace.Output));
    }
}
