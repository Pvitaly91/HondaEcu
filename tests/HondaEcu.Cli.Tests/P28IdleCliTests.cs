...
public sealed class P28IdleCliTests
{
    private static string[] Args(P28AcquisitionCliTests.Workspace w) => ["research", "p28-idle", "target-check", w.Baseline, "--profile", "p28-304", "--confirm-profile", "--baseline-binding", w.Binding, "--runner", w.Runner, "--scenario", w.Scenario, "--output", w.Output];
    [Theory]
    ...
    public async Task ActualRustCliEndToEndWritesHonestUnresolvedAndNotRunWithoutBin()
    {
        ...
        File.WriteAllText(w.Scenario, P28IdleScenario.Create(new(9, 8, 7, 1), [new(0, 100, 300), new(1, 100, 300)], "Invented CLI native stop").ToJson());
