...
{
    private async Task<int> P28VtecStateCheckAsync(string[] args, CancellationToken cancellationToken)
    {
        ...
        command.RequirePositionals(1, "hondaecu research p28-vtec state-check <baseline> --profile p28-304 --confirm-profile --baseline-binding <private-json> --runner <rust-executable> --scenario <private-json> --output <new-private-json>");
        if (!command.HasFlag("confirm-profile")) throw new CliUsageException("Stateful execution requires --confirm-profile.");
        IReadOnlyList<string> assumptions;
        try { assumptions = P28StatefulValidator.ValidateAssumptions(command.Many("allow-assumption")); }
        ...
        var scenario = P28StatefulScenario.Parse(utf8.GetString(snapshot[scenarioPath]));
        if (profile.SourcePath is { } profilePath && P28VtecInspector.ComputeProfileDigest(profile) !=
            ...
            throw new InvalidDataException("Selected profile changed while loading stateful inputs.");
        ...
        var report = await P28StatefulValidator.ExecuteAsync(baseline, profile, binding, true, runnerPath, scenario, assumptions, derived,
        ...
        foreach (var s in report.Sequences)
            await _output.WriteLineAsync($"{s.ImageId}/{s.ScratchPattern}: {s.StrictMatches} strict matches; {s.ConditionalMatches} conditional; {s.Unresolved} unresolved; {s.NotRun} NotRun; stop call={s.StopCallIndex}.").ConfigureAwait(false);
        await _output.WriteLineAsync("Stateful software boundary before 0x12FC; raw compactCode, explicit native counter schedule, no physical RPM/time or actuator claim.").ConfigureAwait(false);
