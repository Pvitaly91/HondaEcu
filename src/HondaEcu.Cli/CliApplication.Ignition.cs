...
{
    private async Task<int> P28IgnitionResearchAsync(string[] args, CancellationToken cancellationToken)
    {
        ...
        if (args.Length == 0 || args[0] is not ("maps-inspect" or "lookup-check"))
            throw new CliUsageException("Usage: hondaecu research p28-ignition <maps-inspect|lookup-check|export> ...");
        ...
        command.RequirePositionals(1, check
            ? "hondaecu research p28-ignition lookup-check <baseline.bin> --profile p28-304 --confirm-profile --baseline-binding <binding.json> --runner <rust-runner> --scenario <scenario.json> --output <new-private-validation.json>"
            : "hondaecu research p28-ignition maps-inspect <baseline.bin> --profile p28-304 [--confirm-profile --baseline-binding <binding.json>] --output <new-private-map-report.json>");
        if (check && !command.HasFlag("confirm-profile"))
            throw new CliUsageException("Ignition-map native execution requires --confirm-profile and exact binding.");
        ...
        if (profile.SourcePath is { } profilePath && P28VtecInspector.ComputeProfileDigest(profile) !=
            ...
            throw new InvalidDataException("Profile changed while loading ignition-map inputs.");
        ...
        if (check)
        {
            var scenario = P28IgnitionMapScenario.Parse(utf8.GetString(snapshot[scenarioPath!]));
            var validation = await P28IgnitionMapValidator.ExecuteAsync(baseline, profile, binding!, true, runnerPath!, scenario,
            ...
            foreach (var image in validation.Images) foreach (var sequence in image.Sequences)
                    await _output.WriteLineAsync($"ignition image={image.Image}, scratch={sequence.ScratchPattern}: calls={sequence.Checkpoints.Count}, strict={sequence.Checkpoints.Count(row => row.Disposition == "StrictMatch")}, other={sequence.Checkpoints.Count(row => row.Disposition != "StrictMatch")}").ConfigureAwait(false);
        ...
        {
            var inspection = P28IgnitionMapInspector.Inspect(baseline, profile, binding, command.HasFlag("confirm-profile"));
            ...
            if (inspection.InterpretationApplied)
            {
                ...
                {
                    await _output.WriteLineAsync($"{map.Id}: {map.Rows}x{map.Columns} unsigned raw cells 0x{map.Origin:X4}..0x{map.EndInclusive:X4}").ConfigureAwait(false);
                ...
                foreach (var axis in inspection.Axes)
                    await _output.WriteLineAsync($"{axis.Id}: {string.Join(' ', axis.RawValues)}").ConfigureAwait(false);
            }
            else await _output.WriteLineAsync("Ignition-map interpretation not applied; general image identity only.").ConfigureAwait(false);
        }
        RequireCaptureInputSnapshot(snapshot);
        await WriteJsonFileAsync(outputPath, report, cancellationToken).ConfigureAwait(false);
        await _output.WriteLineAsync("Read-only ignition-map research. PcInspectionOnly / NotFlashReady; units raw and physical RPM/degrees unavailable; GUI r3 paused/NotRun; D1 interactive GUI acceptance NotRun; hardware/full boot NotRun. No BIN written.").ConfigureAwait(false);
