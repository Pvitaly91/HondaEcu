...
public sealed partial class CliApplication
{
    private async Task<int> ResearchAsync(string[] args, CancellationToken cancellationToken)
    {
        if (args.Length == 0)
        {
            throw new CliUsageException("Usage: hondaecu research p28-vtec <inspect|plan|apply|verify|execute-check|producer-check|rpm-preview|checksum-check|compensation-check|checksum-export-plan|checksum-export-apply|checksum-export-verify|checksum-export-inspect> ...");
        }
    ...
    private async Task<int> P28VtecResearchAsync(string[] args, CancellationToken cancellationToken)
    {
        ...
        return args[0] switch
        {
            ...
            "state-check" => await P28VtecStateCheckAsync(args[1..], cancellationToken).ConfigureAwait(false),
            "chain-check" => await P28VtecChainCheckAsync(args[1..], cancellationToken).ConfigureAwait(false),
