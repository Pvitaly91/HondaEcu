...
pub fn validate_request(r: &Request) -> Result<(), String> {
    let s = r.shared_calibration_chain.as_ref().ok_or("M2h stimulus required")?;
    ...
    if s.format_version != 1
        ...
        || a.ignition_load_index > 8 || a.fuel_load_index > 8
        || a.map0_rpm_index > 18 || a.map1_rpm_index > 18
        || s.calls.iter().enumerate().any(|(i,c)| c.index as usize != i
            || c.decision.index != c.index || c.decision.context > 1
            || c.decision.fast_ticks > 32 || c.decision.slow_ticks > 32
        ...
        || r.synthetic.is_some() || r.producer_cases.is_some()
        || r.allow_assumptions.iter().any(|a| a != stateful_forms::SUBB_OFF_ASSUMPTION)
...
fn axes_contract() -> SliceContract {
    SliceContract {
        entry_pc:0x0A0C, exit_pcs:vec![0x0A77],
        code_ranges:vec![[0x0A0C,0x0A77],[0x59B2,0x59E4]],
        psw:0x0101,lrb:0x40,usp:0x180,instruction_budget:768,
        data_seeds:vec![],output_addresses:vec![],program_read_range:None,
    }
}

fn axis_admission(d: &Decoded) -> FormAdmission {
    if ignition::admission(d) == FormAdmission::Allowed || fuel::admission(d) == FormAdmission::Allowed {
        FormAdmission::Allowed
    } else { FormAdmission::Unsupported }
}
...
fn snapshot(cpu: &Cpu,bus: &mut Bus) -> State {
    State {
        ...
        factor0247:read_data_u8(cpu,bus,0x247),output0248:read_data_u8(cpu,bus,0x248),
        factor013f:read_data_u8(cpu,bus,0x13F),output0140:read_data_u16(cpu,bus,0x140),
...
fn seed(cpu: &mut Cpu,bus: &mut Bus,s: &Stimulus) {
    ...
    for (address,value) in stateful::STATE_ADDRESSES.into_iter().zip(s.initial.vtec.bytes()) {
        if address != 0x22 { write_data_u8(cpu,bus,address,value); }
    }
    let a=&s.initial.axes;
    for (address,value) in [
        (0x1BE,a.ignition_load_fraction),(0x1C0,a.fuel_load_fraction),
        (0x1C2,a.map0_rpm_fraction),(0x1C4,a.map1_rpm_fraction),
        (0x140,s.initial.output0140),
    ] { write_data_u16(cpu,bus,address,value); }
    for (address,value) in [
        (0x1BB,a.ignition_load_index),(0x1BC,a.fuel_load_index),
        (0x1C6,a.map0_rpm_index),(0x1C7,a.map1_rpm_index),
        (0x227,s.initial.selector0227),(0x247,s.initial.factor0247),
        (0x248,s.initial.output0248),(0x13F,s.initial.factor013f),
    ...
    for address in [0xB8,0xBC,0x11E,0x120,0x121,0x212,0x214,0x218,0x219,0x21D,0x21F] {
...
fn sequence(rom:&[u8],image_index:usize,pattern:u8,s:&Stimulus,allowed:&[&str]) -> Sequence {
    ...
    let mut seq=Sequence { image_index,scratch_pattern:pattern,checkpoints:Vec::with_capacity(s.calls.len()),completed_calls:0,stop_call_index:-1 };
    ...
    for input in &s.calls {
        ...
        let mut cp=Checkpoint {
            ...
            ignition_origin:None,ignition_lookup_entry:None,ignition_lookup:None,
        ...
        if seq.stop_call_index >= 0 { seq.checkpoints.push(cp); continue; }
        cp.input=Some(input.clone());
        let d=&input.decision;
        for (address,value) in [
            (0x3C7,input.source03c7),(0x238,input.raw_map0_rpm),
            (0xC2,input.raw_map1_rpm),(0xBF,input.raw_load),
        ...
        for (entry,exit,target) in stateful::tick_schedule(d.fast_ticks,d.slow_ticks) {
            ...
            let result=execute_in_state_observed(&mut cpu,&mut bus,&c,&[],false,Some(stateful_forms::admission),true);
        ...
        if cp.status == 0 {
            ...
            if !ignition_selector::supported_caller(&cpu,&mut bus) || !vtec_fuel::supported_caller(&cpu,&mut bus) {
                cp.status=1;cp.error=Some("producer left unsupported joint direct caller gates".into());
            }
        }
        if cp.status == 0 {
        ...
        if cp.status == 0 {
            ...
            cp.status=x.result.status;cp.error=x.result.error.clone();cp.ignition_selection=Some(x);
            if cp.status==0 { cp.ignition_selection_exit=Some(boundary(&cpu,&mut bus)); }
        }
        if cp.status == 0 {
            ...
            cp.status=x.result.status;cp.error=x.result.error.clone();cp.ignition_lookup=Some(x);
            if cp.status==0 { cp.ignition_lookup_exit=Some(boundary(&cpu,&mut bus)); }
        }
        if cp.status == 0 {
            ...
            cp.status=x.result.status;cp.error=x.result.error.clone();cp.ignition_consumer=Some(x);
            if cp.status==0 {
        ...
        if cp.status == 0 {
            ...
            bus.begin_write_journal();bus.start_decision_observer();
            let result=execute_in_state_observed(&mut cpu,&mut bus,&decision_contract,allowed,trace,Some(stateful_forms::admission),true);
        ...
        if cp.status == 0 {
            ...
            if cpu.pc != 0x12FC || !vtec_fuel::supported_caller(&cpu,&mut bus) {
                cp.status=1;cp.error=Some("decision did not reach supported continuous fuel caller boundary".into());
            }
        }
        if cp.status == 0 {
        ...
        } else { seq.stop_call_index=input.index as i32; }
...
pub fn run(r:Request,mut response:Response) -> Result<Response,String> {
    let stimulus=r.shared_calibration_chain.as_ref().ok_or("missing M2h stimulus")?;
    let allowed:Vec<_>=r.allow_assumptions.iter().map(String::as_str).collect();
    response.entry_contracts=entry_contracts();
    response.shared_calibration_sequences=Some(r.images.iter().enumerate().flat_map(|(index,image)|
        r.scratch_patterns.iter().map(|pattern|sequence(&image.rom,index,*pattern,stimulus,&allowed)).collect::<Vec<_>>()).collect());
    Ok(response)
}
